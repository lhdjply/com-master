...
MmsError
mmsServer_callVariableListChangedHandler(MmsVariableListAccessType accessType, MmsVariableListType listType,
                                         MmsDomain* domain,
    ...
    for(i = 0; i < numberItems; i++)
    {
      if(request->listOfVariableListName->list.array[i]->present == ObjectName_PR_domainspecific)
      {
        ...
        if(domain)
        {
          ...
          if(variableList)
          {
            ...
            {
              MmsError deleteError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_DELETE, MMS_DOMAIN_SPECIFIC, domain,
                                                                              listName, connection);
      ...
      else if(request->listOfVariableListName->list.array[i]->present == ObjectName_PR_aaspecific)
      {
        ...
        if(variableList)
        {
          ...
          MmsError deleteError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_DELETE, MMS_ASSOCIATION_SPECIFIC, NULL,
                                                                          listName, connection);
      ...
      else if(request->listOfVariableListName->list.array[i]->present == ObjectName_PR_vmdspecific)
      {
        ...
        if(variableList)
        {
          ...
          MmsError deleteError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_DELETE, MMS_VMD_SPECIFIC, NULL, listName,
                                                                          connection);
                        ...
                        char * variableListName, MmsError* mmsError)
{
  ...
  for(i = 0; i < variableCount; i++)
  {
    ...
    if(varSpec->present == VariableSpecification_PR_name)
    {
      ...
      if(DEBUG_MMS_SERVER)
        printf("MMS SERVER: add named variable list entry: %s/%s(%li)%s\n", MmsDomain_getName(elementDomain), variableName,
               arrayIndex, componentName);
  ...
  ByteBuffer* response)
{
  ...
  if(request->variableListName.present == ObjectName_PR_domainspecific)
  {
    ...
    if(LinkedList_size(domain->namedVariableLists) < CONFIG_MMS_MAX_NUMBER_OF_DOMAIN_SPECIFIC_DATA_SETS)
    {
      ...
      else
      {
        ...
        {
          mmsError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_CREATE, MMS_DOMAIN_SPECIFIC, domain, variableListName,
                                                              connection);
  ...
  else if(request->variableListName.present == ObjectName_PR_aaspecific)
  {
    ...
    if(LinkedList_size(connection->namedVariableLists) < CONFIG_MMS_MAX_NUMBER_OF_ASSOCIATION_SPECIFIC_DATA_SETS)
    {
      ...
      else
      {
        ...
        if(namedVariableList)
        {
          if(mmsServer_callVariableListChangedHandler(MMS_VARLIST_CREATE, MMS_ASSOCIATION_SPECIFIC, NULL, variableListName,
                                                      connection) == MMS_ERROR_NONE)
          {
  ...
  else if(request->variableListName.present == ObjectName_PR_vmdspecific)
  {
    ...
    if(LinkedList_size(vmdScopeNVLs) < CONFIG_MMS_MAX_NUMBER_OF_VMD_SPECIFIC_DATA_SETS)
    {
      ...
      if(mmsServer_getNamedVariableListWithName(MmsDevice_getNamedVariableLists(connection->server->device),
                                                variableListName))
      {
        mmsMsg_createServiceErrorPdu(invokeId, response, MMS_ERROR_DEFINITION_OBJECT_EXISTS);
      }
  ...
  ByteBuffer* response)
{
  ...
  if(request->present == ObjectName_PR_domainspecific)
  {
    ...
    if(domain != NULL)
    {
      ...
      if(varList)
      {
        MmsError accessError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_GET_DIRECTORY, MMS_DOMAIN_SPECIFIC, domain,
                                                                        varList->name, connection);
  ...
  else if(request->present == ObjectName_PR_aaspecific)
  {
    ...
    if(varList)
    {
      MmsError accessError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_GET_DIRECTORY, MMS_ASSOCIATION_SPECIFIC,
                                                                      NULL, varList->name, connection);
  ...
  else if(request->present == ObjectName_PR_vmdspecific)
  {
    ...
    if(varList)
    {
      MmsError accessError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_GET_DIRECTORY, MMS_VMD_SPECIFIC, NULL,
                                                                      varList->name, connection);
