  ...
  for (i = 0; i < variableCount; i++)
  {
    ...
    if (varSpec.present == VariableSpecification_PR_name)
    {
      ...
      else if (varSpec.choice.name.present == ObjectName_PR_vmdspecific)
      {
        ...
        MmsVariableSpecification* namedVariable = MmsDevice_getNamedVariable(MmsServer_getDevice(connection->server),
                                                                             nameIdStr);
...
static void
addNamedVariableToNamedVariableListResultList(MmsVariableSpecification* namedVariable, MmsDomain* domain,
                                              char * nameIdStr,
  ...
  ByteBuffer* response)
{
      ...
      ObjectName_PR_domainspecific)
  {
    ...
    else
    {
      ...
      if (namedList)
      {
        MmsError accessError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_READ, MMS_DOMAIN_SPECIFIC, domain,
  ...
  else if (read->variableAccessSpecification.choice.variableListName.present == ObjectName_PR_vmdspecific)
  {
    ...
    MmsNamedVariableList namedList = mmsServer_getNamedVariableListWithName(connection->server->device->namedVariableLists,
                                                                            listName);
    ...
    {
      MmsError accessError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_READ, MMS_VMD_SPECIFIC, NULL,
                                                                      namedList->name, connection);
           ...
           ObjectName_PR_aaspecific)
  {
    ...
    {
      MmsError accessError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_READ, MMS_ASSOCIATION_SPECIFIC, NULL,
                                                                      namedList->name, connection);
