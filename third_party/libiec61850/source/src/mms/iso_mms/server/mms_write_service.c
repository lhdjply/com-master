  ...
  ByteBuffer* response)
{
  if (writeRequest->variableAccessSpecification.choice.variableListName.present == ObjectName_PR_domainspecific)
  {
    ...
    mmsMsg_copyAsn1IdentifierToStringBuffer(
      writeRequest->variableAccessSpecification.choice.variableListName.choice.domainspecific.itemId,
      nameIdStr, 65);
    ...
    else
    {
      ...
      if (namedList)
      {
        MmsError accessError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_WRITE, MMS_DOMAIN_SPECIFIC, domain,
  ...
  else if (writeRequest->variableAccessSpecification.choice.variableListName.present == ObjectName_PR_vmdspecific)
  {
    ...
    mmsMsg_copyAsn1IdentifierToStringBuffer(
      writeRequest->variableAccessSpecification.choice.variableListName.choice.vmdspecific,
      listName, 65);
  ...
  else if (writeRequest->variableAccessSpecification.choice.variableListName.present == ObjectName_PR_aaspecific)
  {
    ...
    mmsMsg_copyAsn1IdentifierToStringBuffer(
      writeRequest->variableAccessSpecification.choice.variableListName.choice.aaspecific,
      listName, 65);
    ...
    if (namedList)
    {
      MmsError accessError = mmsServer_callVariableListChangedHandler(MMS_VARLIST_WRITE, MMS_ASSOCIATION_SPECIFIC, NULL,
                                                                      namedList->name, connection);
