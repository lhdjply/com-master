...
static LinkedList
addSubNamedVaribleNamesToList(MmsServerConnection connection, LinkedList nameList, MmsDomain* domain, char * prefix,
  ...
  if(variable->type == MMS_STRUCTURE)
  {
    ...
    if(index)
    {
      ...
      for(i = 0; i < variable->typeSpec.structure.elementCount; i++)
      {
        ...
        if(variableName)
        {
          bool accessAllowed = mmsServer_checkListAccess(connection->server, MMS_GETNAMELIST_DATA, domain, variableName,
                                                         connection);
...
getJournalListDomainSpecific(MmsServerConnection connection, char * domainName)
{
  ...
  if(domain)
  {
    ...
    {
      allowAccess = connection->server->getNameListHandler(connection->server->getNameListHandlerParameter,
                                                           MMS_GETNAMELIST_JOURNALS, domain, connection);
    }
    ...
    if(allowAccess)
    {
      ...
      if(domain->journals != NULL)
      {
        ...
        while((journalList = LinkedList_getNext(journalList)) != NULL)
        {
          ...
          allowAccess = mmsServer_checkListAccess(connection->server, MMS_GETNAMELIST_JOURNALS, domain, journal->name,
                                                  connection);
...
getNameListDomainSpecific(MmsServerConnection connection, char * domainName)
{
  ...
  if(domain)
  {
    ...
    {
      allowAccess = connection->server->getNameListHandler(connection->server->getNameListHandlerParameter,
                                                           MMS_GETNAMELIST_DATA, domain, connection);
    }
    ...
    if(allowAccess)
    {
      ...
      for(i = 0; i < domain->namedVariablesCount; i++)
      {
        bool accessAllowed = mmsServer_checkListAccess(connection->server, MMS_GETNAMELIST_DATA, domain,
                                                       variables[index[i]]->name, connection);
...
createStringsFromNamedVariableList(LinkedList variableLists, MmsServerConnection connection, MmsDomain* domain)
{
  ...
  while(variableListsElement)
  {
    ...
    bool accessAllowed = mmsServer_checkListAccess(connection->server, MMS_GETNAMELIST_DATASETS, domain, variableList->name,
                                                   connection);
...
getNamedVariableListsDomainSpecific(MmsServerConnection connection, char * domainName)
{
  ...
  if(domain)
  {
    ...
    {
      allowAccess = connection->server->getNameListHandler(connection->server->getNameListHandlerParameter,
                                                           MMS_GETNAMELIST_DATASETS, domain, connection);
    }
  ...
  ByteBuffer* response)
{
  ...
  else if(objectScope == OBJECT_SCOPE_VMD)
  {
    ...
    if(objectClass == OBJECT_CLASS_DOMAIN)
    {
      ...
      {
        allowAccess = connection->server->getNameListHandler(connection->server->getNameListHandlerParameter,
                                                             MMS_GETNAMELIST_DOMAINS, NULL, connection);
      }
    ...
    else if(objectClass == OBJECT_CLASS_NAMED_VARIABLE)
    {
      ...
      {
        allowAccess = connection->server->getNameListHandler(connection->server->getNameListHandlerParameter,
                                                             MMS_GETNAMELIST_DATA, NULL, connection);
      }
    ...
    else if(objectClass == OBJECT_CLASS_NAMED_VARIABLE_LIST)
    {
      ...
      {
        allowAccess = connection->server->getNameListHandler(connection->server->getNameListHandlerParameter,
                                                             MMS_GETNAMELIST_DATASETS, NULL, connection);
      }
