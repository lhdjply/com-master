...
static void
mmsFileReadHandler(uint32_t invokeId, void * parameter, MmsError mmsError, int32_t frsmId, uint8_t * buffer,
                   uint32_t bytesReceived, bool moreFollows)
{
  (void)invokeId;
  (void)frsmId;
  (void)moreFollows;
        ...
        case 0x49: /* file-read-response */
          {
            ...
            if(fileTask)
            {
              ...
              {
                if(mmsMsg_parseFileReadResponse(buffer, startBufPos, maxBufPos, invokeId, fileTask->frmsId, &moreFollows,
                                                mmsFileReadHandler, (void *) fileTask))
                {
                  if(moreFollows)
                  {
                    fileTask->state = MMS_FILE_UPLOAD_STATE_SEND_FILE_READ;
