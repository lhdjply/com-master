...
static MmsValue *
MmsValue_decodeMmsDataRecursive(uint8_t * buffer, int bufPos, int bufferLength, int * endBufPos, int depth,
                                int maxDepth)
{
  depth++;
    ...
    case 0xa2: /* MMS_STRUCTURE */
      {
        ...
        for(i = 0; i < elementCount; i++)
        {
          ...
          MmsValue* elementValue = MmsValue_decodeMmsDataRecursive(buffer, bufPos, bufPos + elementBufLength, NULL, depth,
                                                                   maxDepth);
