...
parseNamedVariableAttributes(GetNamedVariableListAttributesResponse_t * response, bool* deletable)
{
  ...
  for(i = 0; i < attributesCount; i++)
  {
      ...
      if(alternateAccess->list.count > 0)
      {
        if(alternateAccess->list.array[0]->choice.unnamed->present == AlternateAccessSelection_PR_selectAlternateAccess)
        {
             ...
             == AlternateAccessSelection__selectAlternateAccess__accessSelection_PR_index)
          {
            ...
            AlternateAccess_t * nestedAltAccess =
    ...
    MmsVariableAccessSpecification* listEntry = MmsVariableAccessSpecification_createAlternateAccess(domainId, itemId,
                                                                                                     arrayIdx, componentName);
  ...
  bool associationSpecific)
{
  ...
  while(i < listSize)
  {
    ...
    if(variableSpec->arrayIndex != -1)
    {
      ...
      {
        request->listOfVariable.list.array[i]->alternateAccess = mmsClient_createAlternateAccessIndexComponent(
                                                                   variableSpec->arrayIndex, variableSpec->componentName);
      }
      else
      {
