...
static void
updateGenericTrackingObjectValues(MmsMapping* self, ControlObject* controlObject, IEC61850_ServiceType serviceType,
                                  IEC61850_ServiceError errVal)
{
    ...
    case STATE_WAIT_FOR_SELECT:
      {
        ...
        CheckHandlerResult checkHandlerResult = controlObject->checkHandler((ControlAction) controlObject,
                                                                            controlObject->checkHandlerParameter,
        ...
        if(checkHandlerResult != CONTROL_WAITING_FOR_SELECT)
        {
          ...
          else if(controlObject->ctlModel == 4)
          {
            ...
            else
            {
              ...
              MmsServerConnection_sendWriteResponse(controlObject->mmsConnection, controlObject->operateInvokeId,
                                                    (MmsDataAccessError) checkHandlerResult, false);
    ...
    case STATE_WAIT_FOR_EXECUTION:
      {
        ...
        {
          dynamicCheckResult = controlObject->waitForExecutionHandler((ControlAction) controlObject,
                                                                      controlObject->waitForExecutionHandlerParameter, controlObject->ctlVal,
...
#if (CONFIG_IEC61850_SERVICE_TRACKING == 1)
            updateGenericTrackingObjectValues(self, controlObject, IEC61850_SERVICE_TYPE_OPERATE,
                                              IEC61850_SERVICE_ERROR_ACCESS_NOT_ALLOWED_IN_CURRENT_STATE);
#endif /* (CONFIG_IEC61850_SERVICE_TRACKING == 1) */
          }
          else
          {
...
#if (CONFIG_IEC61850_SERVICE_TRACKING == 1)
            updateGenericTrackingObjectValues(self, controlObject, IEC61850_SERVICE_TYPE_COMMAND_TERMINATION,
                                              IEC61850_SERVICE_ERROR_NO_ERROR);
#endif /* (CONFIG_IEC61850_SERVICE_TRACKING == 1) */
    ...
    case STATE_OPERATE:
      {
        ...
        ControlHandlerResult result = operateControl(controlObject, controlObject->ctlVal, currentTime,
                                                     controlObject->testMode);
...
#if (CONFIG_IEC61850_SERVICE_TRACKING == 1)
            updateGenericTrackingObjectValues(self, controlObject, IEC61850_SERVICE_TYPE_COMMAND_TERMINATION,
                                              IEC61850_SERVICE_ERROR_FAILED_DUE_TO_SERVER_CONSTRAINT);
#endif /* (CONFIG_IEC61850_SERVICE_TRACKING == 1) */
...
ControlObject_initialize(ControlObject* self)
{
  ...
  char * sboTimeoutName = StringUtils_createStringInBuffer(strBuf, 130, 4, self->lnName, "$CF$", self->name,
                                                           "$sboTimeout");
  ...
  daName = StringUtils_createStringInBuffer(strBuf, 130, 6, self->mmsDomain->domainName, "/", self->lnName, ".",
                                            self->name, ".Oper.ctlVal");
  ...
  char * stSeldName = StringUtils_createStringInBuffer(strBuf, 130, 6, self->mmsDomain->domainName, "/", self->lnName,
                                                       ".", self->name, ".stSeld");
  ...
  char * opRcvdName = StringUtils_createStringInBuffer(strBuf, 130, 6, self->mmsDomain->domainName, "/", self->lnName,
                                                       ".", self->name, ".opRcvd");
  ...
  char * opOkName = StringUtils_createStringInBuffer(strBuf, 130, 6, self->mmsDomain->domainName, "/", self->lnName, ".",
                                                     self->name, ".opOk");
  ...
  char * tOpOkName = StringUtils_createStringInBuffer(strBuf, 130, 6, self->mmsDomain->domainName, "/", self->lnName, ".",
                                                      self->name, ".tOpOk");
...
updateControlParameters(ControlObject* controlObject, MmsValue* ctlVal, MmsValue* ctlNum, MmsValue* origin,
                        bool synchroCheck, bool interlockCheck)
{
