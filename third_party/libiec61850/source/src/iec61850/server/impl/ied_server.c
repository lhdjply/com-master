                                     ...
                                     char * objectReference, int position, int idx, char * componentId, int compIdPos)
{
  ...
  if(dataAttribute->elementCount > 0)
  {
    ...
    {
      installDefaultValuesForDataAttribute(self, ld, subDataAttribute, objectReference, childPosition, subIdx, componentId,
                                           childIdPos);
  ...
  else
  {
    ...
    while(subDataAttribute)
    {
      installDefaultValuesForDataAttribute(self, ld, subDataAttribute, objectReference, childPosition, idx, componentId,
                                           childIdPos);
                                  ...
                                  char * objectReference, int position, int idx, char * componentId, int compIdPos)
{
  ...
  if(dataObject->elementCount > 0)
  {
    ...
    while(arrayElemNode)
    {
      installDefaultValuesForDataObject(self, ld, (DataObject *)arrayElemNode, objectReference, childPosition, arrayIdx,
                                        componentId, compIdPos);
  ...
  while(childNode)
  {
    ...
    {
      installDefaultValuesForDataObject(self, ld, (DataObject *) childNode, objectReference, childPosition, idx, componentId,
                                        childCompIdPos);
    }
    else if(childNode->modelType == DataAttributeModelType)
    {
      installDefaultValuesForDataAttribute(self, ld, (DataAttribute *) childNode, objectReference, childPosition, idx,
                                           componentId, childCompIdPos);
    }
...
void
IedServer_handleWriteAccessForComplexAttribute(IedServer self, DataAttribute* dataAttribute, WriteAccessHandler handler,
                                               void * parameter)
{
  if(dataAttribute == NULL)
  {
