    ...
    else
    {
      ...
      else
      {
        if(MmsValue_getType(value) == MMS_DATA_ACCESS_ERROR)
        {
          ...
          handler(invokeId, call->callbackParameter,
                  iedConnection_mapDataAccessErrorToIedError(MmsValue_getDataAccessError(value)), NULL);
        }
        else
        {
          ClientGooseControlBlock returnGoCB = updateGoCB;
...
writeVariableHandler(uint32_t invokeId, void * parameter, MmsError mmsError, MmsDataAccessError accessError)
{
  ...
  if(call)
  {
    ...
    else
    {
      ...
      MmsConnection_writeVariableAsync(self->connection, &(call->invokeId), &writeError, param->domainId, itemId, value,
                                 ...
                                 uint32_t parametersMask, bool singleRequest, IedConnection_GenericServiceHandler handler, void * parameter)
{
  *error = IED_ERROR_OK;
  ...
  {
    struct sWriteGoCBVariablesParameter* param = (struct sWriteGoCBVariablesParameter *) GLOBAL_MALLOC(sizeof(
                                                                                                         struct sWriteGoCBVariablesParameter));
