...
createNewConnectionObject(TLSConfiguration tlsConfig, bool useThreads)
{
  ...
  if (self)
  {
    ...
    self->outstandingCalls = (IedConnectionOutstandingCall) GLOBAL_CALLOC(CONFIG_DEFAULT_MAX_SERV_OUTSTANDING_CALLED,
                                                                          sizeof(struct sIedConnectionOutstandingCall));
...
IedConnection_setMaxOutstandingCalls(IedConnection self, int calling, int called)
{
  ...
  self->outstandingCalls = (IedConnectionOutstandingCall)GLOBAL_CALLOC(called,
                                                                       sizeof(struct sIedConnectionOutstandingCall));
                                            ...
                                            FunctionalConstraint fc, IedConnection_GetVariableSpecificationHandler handler, void * parameter)
{
  ...
  MmsConnection_getVariableAccessAttributesAsync(self->connection, &(call->invokeId), &err, domainId, itemId,
                                                 getAccessAttrHandler, self);
                                            ...
                                            IedConnection_GetNameListHandler handler, void * parameter)
{
  ...
  MmsConnection_getDomainVariableListNamesAsync(self->connection, &(call->invokeId), &err, ldName, continueAfter, result,
                                                getNameListHandler, self);
                              ...
                              IedConnection_ReadObjectHandler handler, void * parameter)
{
  *error = IED_ERROR_OK;
  ...
  if (brace)
  {
    ...
    if (secondBrace)
    {
      ...
      if (endPtr == secondBrace)
      {
        ...
        *brace = 0;
        ...
        MmsConnection_readSingleArrayElementWithComponentAsync(self->connection, &(call->invokeId), &err, domainId, itemId,
                                                               index, component, readObjectHandlerInternal, self);
      }
      else
    ...
    MmsConnection_readVariableAsync(self->connection, &(call->invokeId), &err, domainId, itemId, readObjectHandlerInternal,
                                    self);
    ...
    if (secondBrace)
    {
      ...
      if (endPtr == secondBrace)
      {
        ...
        *brace = 0;
        ...
        value = MmsConnection_readSingleArrayElementWithComponent(self->connection, &mmsError, domainId, itemId, index,
                                                                  component);
      }
      else
        *error = IED_ERROR_USER_PROVIDED_INVALID_ARGUMENT;
    ...
    if (secondBrace)
    {
      ...
      if (endPtr == secondBrace)
      {
        ...
        *brace = 0;
        ...
        MmsConnection_writeSingleArrayElementWithComponent(self->connection, &mmsError, domainId, itemId, index, component,
                                                           value);
                               ...
                               FunctionalConstraint fc, MmsValue* value, IedConnection_GenericServiceHandler handler, void * parameter)
{
  *error = IED_ERROR_OK;
    ...
    if (secondBrace)
    {
      ...
      if (endPtr == secondBrace)
      {
        ...
        *brace = 0;
        ...
        MmsConnection_writeSingleArrayElementWithComponentAsync(self->connection, &(call->invokeId), &err, domainId, itemId,
  ...
  {
    MmsConnection_writeVariableAsync(self->connection, &(call->invokeId), &err, domainId, itemId, value,
                                     writeVariableHandler, self);
...
IedConnection_getDeviceModelFromServer(IedConnection self, IedClientError* error)
{
  ...
  else if (error) *error = iedConnection_mapMmsErrorToIedError(mmsError);
}
...
static void
fileDirectoryHandlerEx(uint32_t invokeId, void * parameter, MmsError err, char * filename, uint32_t size,
                       uint64_t lastModfified,
                       bool moreFollows)
{
  IedConnection self = (IedConnection) parameter;
  ...
  if (call)
  {
    if (call->specificParameter2.getFileDirectory.cont)
    {
      ...
      call->specificParameter2.getFileDirectory.cont =
        handler(invokeId, call->callbackParameter, iedConnection_mapMmsErrorToIedError(err), filename, size, lastModfified,
                moreFollows);
    }
...
static void
mmsConnectionFileReadHandler (uint32_t invokeId, void * parameter, MmsError mmsError, int32_t frsmId, uint8_t * buffer,
                              uint32_t byteReceived,
                              bool moreFollows)
{
  IedConnection self = (IedConnection) parameter;
  ...
  if (call)
  {
    ...
    if (mmsError != MMS_ERROR_NONE)
    {
      ...
      {
        /* close file */
        MmsConnection_fileCloseAsync(self->connection, &(call->invokeId), &mmsError, frsmId, mmsConnectionFileCloseHandler,
                                     self);
    ...
    {
      bool cont = handler(call->specificParameter2.getFileInfo.originalInvokeId, call->callbackParameter, IED_ERROR_OK,
                          invokeId, buffer, byteReceived, moreFollows);
...
static void
mmsConnectionFileOpenHandler (uint32_t invokeId, void * parameter, MmsError mmsError, int32_t frsmId, uint32_t fileSize,
                              uint64_t lastModified)
{
  (void)fileSize;
  (void)lastModified;
                           ...
                           IedConnection_GenericServiceHandler handler, void * parameter)
{
  ...
  MmsConnection_obtainFileAsync(self->connection, &(call->invokeId), &err, sourceFilename, destinationFilename,
                                deleteFileAndSetFileHandler, self);
                                 ...
                                 IedConnection_GenericServiceHandler handler, void * parameter)
{
  *error = IED_ERROR_OK;
  ...
  if (isAssociationSpecific)
  {
    MmsConnection_deleteAssociationSpecificNamedVariableListAsync(self->connection, &(call->invokeId), &mmsError, itemId,
                                                                  deleteNamedVariableListHandler, self);
  }
  else
  {
    MmsConnection_deleteNamedVariableListAsync(self->connection, &(call->invokeId), &mmsError, domainId, itemId,
                                               deleteNamedVariableListHandler, self);
  }
...
static void
getDataSetDirectoryAsyncHandler(uint32_t invokeId, void * parameter, MmsError mmsError,
                                       ...
                                       IedConnection_GetDataSetDirectoryHandler handler, void * parameter)
{
  ...
  if (isAssociationSpecific)
    MmsConnection_readNamedVariableListDirectoryAssociationSpecificAsync(self->connection, &(call->invokeId), &mmsError,
                                                                         itemId, getDataSetDirectoryAsyncHandler, self);

  else
    MmsConnection_readNamedVariableListDirectoryAsync(self->connection, &(call->invokeId), &mmsError, domainId, itemId,
                                                      getDataSetDirectoryAsyncHandler, self);
                                 ...
                                 LinkedList/*<MmsValue*>*/ values, /* OUTPUT */LinkedList* /* <MmsValue*> */accessResults)
{
  ...
  MmsConnection_writeNamedVariableList(self->connection, &mmsError, isAssociationSpecific, domainId, itemId, values,
                                       accessResults);
...
static void
writeDataSetHandlerInternal(uint32_t invokeId, void * parameter, MmsError err,
                                      ...
                                      LinkedList/*<MmsValue*>*/ values, IedConnection_WriteDataSetHandler handler, void * parameter)
{
  ...
  MmsConnection_writeNamedVariableListAsync(self->connection, &(call->invokeId), &err, isAssociationSpecific, domainId,
                                            itemId, values, writeDataSetHandlerInternal, self);
...
static void
readJournalHandler(uint32_t invokeId, void * parameter, MmsError err, LinkedList /* <MmsJournalEntry> */ journalEntries,
                   bool moreFollows)
{
  IedConnection self = (IedConnection) parameter;
