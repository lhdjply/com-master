...
FileSystem_openDirectory(char * directoryName)
{
  ...
  if(dirHandle->handle != NULL)
  {
    ...
    WideCharToMultiByte(CP_UTF8, 0, dirHandle->findData.cFileName, -1, dirHandle->utf8Filename, (MAX_PATH * 3) + 1, NULL,
                        NULL);
  }
...
getNextDirectoryEntry(DirectoryHandle directory, bool* isDirectory)
{
  ...
  else
  {
    ...
    if(FindNextFileW(directory->handle, &(directory->findData)) != 0)
    {
      ...
      /* convert WCHAR to UTF-8 */
      WideCharToMultiByte(CP_UTF8, 0, directory->findData.cFileName, -1, directory->utf8Filename, (MAX_PATH * 3) + 1, NULL,
                          NULL);
