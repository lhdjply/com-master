...
convertAddressToStr(struct sockaddr_storage* addr)
{
  ...
  if(addr->ss_family == AF_INET)
  {
    ...
    ipv4Addr->sin_port = 0;
    WSAAddressToStringA((LPSOCKADDR)ipv4Addr, sizeof(struct sockaddr_storage), NULL, (LPSTR)addrString,
  ...
  else if(addr->ss_family == AF_INET6)
  {
    ...
    WSAAddressToStringA((LPSOCKADDR)ipv6Addr, sizeof(struct sockaddr_storage), NULL, (LPSTR)addrString,
                        (LPDWORD)&addrStringLen);
    isIPv6 = true;
  }
  else
