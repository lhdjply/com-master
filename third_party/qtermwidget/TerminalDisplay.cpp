...
void TerminalDisplay::fontChange(const QFont &)
{
  ...
  _fontWidth = qRound(static_cast<double>(fm.horizontalAdvance(QLatin1String(REPCHAR))) / static_cast<double>(qstrlen(
                                                                                                                REPCHAR)));
  ...
  if(_terminalSizeHint && isVisible())
  {
    ...
    if(!_resizeWidget)
    {
      ...
      _resizeWidget->setStyleSheet(
        QLatin1String("background-color:palette(window);border-style:solid;border-width:1px;border-color:palette(dark)"));
