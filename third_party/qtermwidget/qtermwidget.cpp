...
void QTermWidget::setMotionAfterPasting(int)
{
  // m_impl->m_session->setMotionAfterPasting();
  // 这个方法在Session类中不存在，暂时注释掉
}

int QTermWidget::historyLinesCount()
{
  return m_impl->m_session->emulation()->lineCount();
}

void QTermWidget::displayData(const QByteArray &data)
{
  // 直接向模拟器发送数据，让它处理显示
