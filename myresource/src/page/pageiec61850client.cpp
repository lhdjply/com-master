...
bool PageIEC61850Client::connect(const QString& hostname, int port)
{
    ...
    for (const QString& device : m_logicalDevices) {
        LinkedList nodes = IedConnection_getLogicalNodeDirectory(m_connection, &error, device.toStdString().c_str(), ACSI_CLASS_DATA_OBJECT);
        if (error == IED_ERROR_OK) {
...
QStringList PageIEC61850Client::getLogicalNodeList(const QString& deviceName)
{
    ...
    IedClientError error;
    LinkedList logicalNodes = IedConnection_getLogicalDeviceDirectory(m_connection, &error, deviceName.toStdString().c_str());
...
bool PageIEC61850Client::writeValue(const QString& objectReference, const QVariant& value, const QString& functionalConstraint)
{
    QMutexLocker locker(&m_mutex);
...
void PageIEC61850Client::setupDataModelTreeUI()
{
    ...
    QObject::connect(m_dataModelTree, &QWidget::customContextMenuRequested, this, &PageIEC61850Client::onContextMenuRequested);
...
void PageIEC61850Client::buildDataModel()
{
    ...
    for (int deviceIndex = 0; deviceIndex < deviceCount; ++deviceIndex) {
        ...
        onDataModelLoadProgress(deviceStartProgress, QString("正在加载设备 %1 (%2/%3)").arg(device).arg(deviceIndex + 1).arg(deviceCount));
        
        QTreeWidgetItem* deviceItem = addLogicalDevice(device);
        
        onDataModelLoadProgress(deviceStartProgress + 2, QString("设备 %1 包含 %2 个逻辑节点").arg(device).arg(logicalNodes.size()));
        ...
        for (int nodeIndex = 0; nodeIndex < nodeCount; ++nodeIndex) {
            ...
            onDataModelLoadProgress(nodeProgress, QString("正在处理节点 %1.%2 (%3/%4)").arg(device).arg(node).arg(processedNodes + 1).arg(totalNodes));
            ...
            for (int objectIndex = 0; objectIndex < objectCount; ++objectIndex) {
                ...
                if (objectIndex % 10 == 0 || objectIndex == objectCount - 1) {
                    ...
                    onDataModelLoadProgress(objectProgress, QString("节点 %1.%2: 加载数据对象 %3/%4").arg(device).arg(node).arg(objectIndex + 1).arg(objectCount));
                }
            }
...
QTreeWidgetItem* PageIEC61850Client::addDataObject(QTreeWidgetItem* parentItem, const QString& objectName, const QString& objectReference, int depth)
{
...
void PageIEC61850Client::setupDataValueWidgetUI()
{
    ...
    QObject::connect(m_readButton, &QPushButton::clicked, this, [this]() { readValue(); });
    QObject::connect(m_writeButton, &QPushButton::clicked, this, [this]() { writeValue(); });
}
